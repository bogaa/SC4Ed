	...
	case WM_LBUTTONDBLCLK:
		{
			...
			if(cPal < 0x100 && nmmx.palCacheOffset[(dPalOverMouse << 4) | cPal] && ChooseColor(&chColor))
			{
				*(LPWORD)(nmmx.rom + nmmx.palCacheOffset[(dPalOverMouse << 4) | cPal]) = nmmx.ConvertRGB2SNES(chColor.rgbResult);
				LPWORD asd = (LPWORD)(nmmx.rom + nmmx.palCacheOffset[(dPalOverMouse << 4) | cPal]);
				asd = asd;
	...
	case WM_PAINT:
		{
			...
			sprintf_s(out, "Offset: 0x%5X   Color %04X", nmmx.palCacheOffset[(dPalOverMouse << 4) + dPalSelected], *(LPWORD)(nmmx.rom + nmmx.palCacheOffset[(dPalOverMouse << 4) + dPalSelected]));
			render.Print(hBack, 2, 33, out, 0);
