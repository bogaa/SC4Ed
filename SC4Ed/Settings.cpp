...
const char configFile[] = "SC4Ed.ini";
...
void LoadSettings()
{
	...
	auto ret = RegOpenKey(HKEY_CURRENT_USER, TEXT("SOFTWARE\\SC4Ed\\"), &key);

	if (ret == ERROR_SUCCESS) {
		size = sizeof(value);
		ret |= RegQueryValueEx(key, TEXT("RomAtStartup"), NULL, NULL, (LPBYTE)&value, &size);
		set.romAtStartup = value;
		size = sizeof(value);
		...
		int v = 0;
		ret |= RegQueryValueEx(key, TEXT("IE_VOLUME"), NULL, NULL, (LPBYTE)&v, &size);
		if (ret == ERROR_SUCCESS)
			set.volume = v;

		v = 0;
		ret |= RegQueryValueEx(key, TEXT("DropX"), NULL, NULL, (LPBYTE)&v, &size);
		if (ret == ERROR_SUCCESS)
			set.dropXOffset = v;

		v = 0;
		ret |= RegQueryValueEx(key, TEXT("DropY"), NULL, NULL, (LPBYTE)&v, &size);
		if (ret == ERROR_SUCCESS)
			set.dropYOffset = v;
...
void SaveSettings()
{
	HKEY key;
	DWORD value = 0;
	auto ret = RegCreateKey(HKEY_CURRENT_USER, TEXT("SOFTWARE\\SC4Ed\\"), &key);
	...
	if (ret == ERROR_SUCCESS) {
		...
		ret |= RegSetValueEx(key, TEXT("IE_VOLUME"), 0, REG_DWORD, (LPBYTE)&set.volume, sizeof(value));

		ret |= RegSetValueEx(key, TEXT("DropX"), 0, REG_DWORD, (LPBYTE)&set.dropXOffset, sizeof(value));
		ret |= RegSetValueEx(key, TEXT("DropY"), 0, REG_DWORD, (LPBYTE)&set.dropYOffset, sizeof(value));
