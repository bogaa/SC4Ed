...
void SetWindowTitle(HWND hWnd, CSTRING fileName)
{
	...
	wsprintf(title, "SC4ED - %s", realFileName);
	SetWindowText(hWnd, title);
	Free(realFileName);
	Free(title);
}
bool OpenDebug(SC4EDCore *SC4EDCore)
{
	return SC4EDCore->LoadNewRom("C:\\debugx.smc");
}
bool OpenFileDialogCore(HWND hWnd, LPCSTR filter, SC4EDCore *SC4EDCore)
{
    OPENFILENAME ofn;
	...
	if (GetOpenFileName(&ofn))
		if (SC4EDCore->LoadNewRom(ofn.lpstrFile))
		{
...
bool SaveFileDialogCore(HWND hWnd, LPCSTR filter, SC4EDCore *romcore)
{
    OPENFILENAME ofn;
    CHAR szFileName[MAX_PATH] = "";
