...
{
	SendMessage(hCtrl, UDM_SETRANGE, min, max);
...
void SpinBox::SetPos(int pos, bool thumb)
{
	SendMessage(hCtrl, UDM_SETPOS, 0, MAKELONG(pos,0));
	if (maxval>>24)
		sprintf_s(text, "%8X", pos);
	else if (maxval>>16)
		sprintf_s(text, "%6X", pos);
	else if (maxval>>12)
		sprintf_s(text, "%4X", pos);
	else if (maxval>>8)
		sprintf_s(text, "%3X", pos);
	else if (maxval>>4)
		sprintf_s(text, "%2X", pos);
	else
		sprintf_s(text, "%1X", pos);
	if (thumb) {
...
{
	return (long)hCtrl == id || (long)hBox == id;
}
